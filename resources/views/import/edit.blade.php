...
@section('title', 'Edit Import Notification')
        ...
        <div class="max-w-6xl mx-auto">
            <div class="bg-white rounded-lg shadow-md">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h1 class="text-2xl font-bold text-gray-800">Edit Import Notification #{{ $importNotification->id }}</h1>
                    <p class="text-gray-600 mt-1">Edit the import notification - Step {{ $stepIndex + 1 }} of
                        {{ count($steps) }}</p>
                </div>
                ...
                <div class="px-6 py-4 border-b border-gray-200">
                    <div class="flex flex-wrap gap-2">
                        @foreach ($steps as $index => $step)
                            <a href="{{ route('import.edit', $importNotification) }}?step={{ $step }}"
                ...
                <!-- Form -->
                <form action="{{ route('import.update', $importNotification) }}" enctype="multipart/form-data"
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                        <div>
                            ...
                            @endphp
                            @if ($prevStep)
                                <a href="{{ route('import.edit', $importNotification) }}?step={{ $prevStep }}"
                        ...
                        <div class="flex gap-2">
                            ...
                            @if ($nextStep)
                                @if ($current === 'dokumen' || $current === 'kemasan' || $current === 'barang')
                                    <a href="{{ route('import.edit', $importNotification) }}?step={{ $nextStep }}"
